...
- 不修改源代码；仅生成/更新文档与 `.cursor/index.json`。
- **忽略规则获取策略**：
...
1.  **写入根级 `ARCHITECTURE.md`**
    ...
    - 根级结构（精简而全局）：
      ...
      - **✨ 新增：模块结构图（Mermaid）**
        ...
        - 每个节点应可点击，并链接到对应模块的 `ARCHITECTURE.md` 文件。
        - 示例语法：
          ...
          graph TD
              ...
              click C "./packages/auth/ARCHITECTURE.md" "查看 auth 模块文档"
              click D "./packages/ui-library/ARCHITECTURE.md" "查看 ui-library 模块文档"
              click F "./services/audit-log/ARCHITECTURE.md" "查看 audit-log 模块文档"
          ```
...
2.  **写入模块级 `ARCHITECTURE.md`**
    - 放在每个模块目录下，结构建议：
      - **✨ 新增：相对路径面包屑**
        - 在每个模块 `ARCHITECTURE.md` 的**最顶部**，插入一行相对路径面包屑，链接到各级父目录及根 `ARCHITECTURE.md`。
        - 示例（位于 `packages/auth/ARCHITECTURE.md`）：
          `[根目录](../../ARCHITECTURE.md) > [packages](../) > **auth**`
      - 模块职责
      - 入口与启动
      - 对外接口
      - 关键依赖与配置
...
- 将"缺口清单"写入 `.cursor/index.json`，下次运行时优先补齐缺口（**断点续扫**）。

## 五、结果摘要（打印到主对话）

- 根/模块 `ARCHITECTURE.md` 新建或更新状态；
- 模块列表（路径+一句话职责）；
- 覆盖率与主要缺口；
...
- 路径使用相对路径；
- 时间信息：使用通过命令参数提供的时间戳，并在 `.cursor/index.json` 中写入 ISO-8601 格式。
