...
| **范围边界** | 仅阶段 1 的 10 项；不实现 type 列（expand/selection/index）、不实现 formatter/render；sortable 仅做配置映射 |
...
| type 列（expand/selection/index） | 阶段 1 过滤，阶段 2 或后续迭代再考虑 |
| formatter/render | 后续迭代，1.1 类型可预留 |
...
| `src/router/routes.ts` | 修改 | 添加 /vtable、/vtable/stage1、/vtable/stage2 |
