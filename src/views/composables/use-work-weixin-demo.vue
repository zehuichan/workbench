...
<template>
  <DemoPage width="wide">
    <template #description>
      ...
      <code>ready</code> 仅在
      <code>onAgentConfigSuccess</code> 后为
      <code>true</code>。非企微环境或
      <code>VITE_WW_JSSDK_ENABLED !== 'true'</code> 时跳过。签名暂走
    ...
    <template #api>
      <DemoApiTable title="Returns (tuple)">
        <tr>
          ...
          <td>
            <code>onAgentConfigSuccess</code> 之后为
            <code>true</code>。
          </td>
        </tr>
        <tr>
          ...
          <td>
            <code>@wecom/jssdk</code> 命名空间；可直接调其 JSAPI。
          </td>
        </tr>
      </DemoApiTable>
    ...
    <DemoBlock title="代码演示">
      <template #hint>
        ...
        <code>wxwork</code>）、
        <code>VITE_WW_JSSDK_ENABLED=true</code>，并配置 corp/agent。
      </template>
