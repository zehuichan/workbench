...
function buildDependencyGraph(columns: ReTableColumn[]): Map<string, Set<string>> {
  const graph = new Map<string, Set<string>>()
  for (const column of columns) {
    if (!column.prop) continue
    const rules = Array.isArray(column.rules) ? column.rules : column.rules ? [column.rules] : []
...
export function useValidation(options: UseValidationOptions) {
  ...
  function mergeRules(prop: string, column: ReTableColumn): RuleItem[] {
  ...
  async function validateRow(
    ...
    try {
      ...
      const validationErrors = e?.errors as Array<{ field: string; message: string }> | undefined
      if (validationErrors) {
  ...
  async function validate(
    ...
    // Clear errors for rows we're about to validate
  ...
  async function validateField(
    ...
    try {
      ...
      const validationErrors = e?.errors as Array<{ message: string }> | undefined
  ...
  function scrollToFirstError(): void {
    ...
    nextTick(() => {
      const rowEl = el.querySelector(`.re-table-row-${minRow}`)
