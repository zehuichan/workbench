...
export interface AdaptiveOptions {
  /** 只读取这一项配置，不需要整个 PlusTableProps——本模块与行数据形状无关，无需泛型 */
  adaptive: () => boolean | AdaptiveConfig | undefined;
...
export function createAdaptive(options: AdaptiveOptions) {
  const { adaptive, gridRef, paginationRef } = options;

  const config = computed<Required<AdaptiveConfig>>(() => {
    const overrides =
      typeof adaptive() === 'object' ? (adaptive() as AdaptiveConfig) : {};
    return {
  ...
  const isContainerMode = computed(
    () => !!adaptive() && config.value.mode === 'container',
  );
