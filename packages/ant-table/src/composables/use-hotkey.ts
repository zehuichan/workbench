...
/** 编辑器内的弹层（select/picker/dropdown）失焦时不应判定为离开表格 */
...
export function useHotkey(options: UseHotkeyOptions) {
  ...
  const overrideHotkeys = computed(
    () => customHotkeys?.value?.filter((h) => h.override) ?? [],
  );
  const normalHotkeys = computed(
    () => customHotkeys?.value?.filter((h) => !h.override) ?? [],
  );
  ...
  useEventListener(wrapperEl, 'focusout', (e: FocusEvent) => {
    ...
    if (related?.closest(POPUP_SELECTOR)) return;
  ...
  function isTargetInEditor(event: KeyboardEvent): boolean {
    ...
    return !!target?.closest('.atbl-cell-editor');
  }

  /** 'all' 模式：编辑器内仅处理 Tab/Enter/Esc，其余交给编辑器 */
  ...
  function handleBuiltinKey(event: KeyboardEvent): boolean {
    ...
    switch (event.key) {
      ...
      case 'Home':
        ...
        else focusCell(Math.max(0, activeRowIndex.value), 0);
        if (isAll) focusActiveEditor();
        return true;

      case 'End':
        event.preventDefault();
        if (event.ctrlKey) focusCell(rowCount - 1, colCount - 1);
        else focusCell(Math.max(0, activeRowIndex.value), colCount - 1);
        if (isAll) focusActiveEditor();
        return true;
